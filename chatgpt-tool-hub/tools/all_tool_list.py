...
from tools.meteo.api_docs_prompts import OPEN_METEO_DOCS
from tools.meteo.meteo_weather import MeteoWeatherTool
from tools.python.python_repl import PythonREPLTool
from tools.terminal.base import Terminal
from tools.web_requests import RequestsGetTool, RequestsWrapper
...
def _get_wolfram_alpha(**kwargs: Any) -> BaseTool:
    from tools.wolfram_alpha import WolframAlphaQueryRun, WolframAlphaAPIWrapper

    return WolframAlphaQueryRun(api_wrapper=WolframAlphaAPIWrapper(**kwargs))


def _get_google_search(**kwargs: Any) -> BaseTool:
    from tools.google_search.google_search import GoogleSearch, GoogleSearchAPIWrapper

    return GoogleSearch(api_wrapper=GoogleSearchAPIWrapper(**kwargs))


def _get_wikipedia(**kwargs: Any) -> BaseTool:
    from tools.wikipedia.wikipedia import WikipediaQuery, WikipediaAPIWrapper

    return WikipediaQuery(api_wrapper=WikipediaAPIWrapper(**kwargs))


def _get_news_api(llm: BaseLLM, **kwargs: Any) -> BaseTool:
    from tools.news import NEWS_DOCS, NewsTool
